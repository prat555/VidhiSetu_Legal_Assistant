...
CATEGORY_MAP = {
    ...
    "criminal": "criminal",
    # Civil & Property
    "civil_property": "civil",
    # Family
    "family": "family",
    # Women & Child Protection
    "women_protection": "women_protection",
    ...
    "cyber_tech": "cyber",
    # Business & Commercial
    "business_commercial": "commercial",
    # Financial & Banking
    ...
    # Judgments (kept)
    "judgments": "civil",
    # Legacy folder names (backward compat)
    "ipc": "criminal",
    "crpc": "criminal",
...
def discover_files(base_dir: Path) -> Iterable[Path]:
    """Yield all supported raw documents recursively."""
...
def infer_category(folder_name: str) -> str:
    ...
    return CATEGORY_MAP.get(folder_name.lower(), "civil")
...
def ingest() -> None:
    """Read docs, chunk, embed, and persist unique chunks into ChromaDB."""
    ...
    for file_path in discover_files(RAW_DOCS_DIR):
        ...
        # Ask Chroma which of the unique IDs already exist in the store.
        ...
        print(
            f"[file] {file_path} | chunks={len(chunks)} | unique={len(unique_texts)} | added={len(new_texts)} | skipped={len(unique_texts) - len(new_texts)}"
        )
